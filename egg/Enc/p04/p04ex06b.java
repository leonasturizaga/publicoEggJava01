...
public static void	rellenarMatriz(String[][] sopa){
	int x;
	for (int i = 0; i < sopa.length; i++) {
		for (int j = 0; j < sopa[0].length; j++) {
			if (sopa[i][j].equalsIgnoreCase("*") ) {
				x =(int)(Math.random()*(9-0+1)+0);
				sopa[i][j]= String.valueOf(x);
			}
		}
	}
}
...
public static void palabraPosicion(String[][] sopa,String[] vec6,int[] posh){
	Scanner leerText = new Scanner(System.in);
	String palabra="";
	int posv=-1;
	int k=0;
		...
		while (valido==false) {
			...
			if (aux[rand-1]==rand) {
				...
				for (int j = posv; j < posv+palabra.length(); j++) {
					sopa[rand-1][j] = palabra.substring(k,k+1); //si la matriz sopa seria String[][]
					k++;
				}
				k=0;
			}
